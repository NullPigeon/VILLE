...
}, required: ['feasibility', 'implementationPlan', 'visualDirection', 'interactionPlan', 'accuracyPlan', 'limitations'], additionalProperties: false };
...
function validateArchitecture(plan) {
  ...
  if (!['SUPPORTED', 'UNSUPPORTED'].includes(plan.feasibility) || !validList(plan.implementationPlan, 3, 8) ||
    ...
    !validList(plan.interactionPlan, 1, 6) || !validList(plan.accuracyPlan, 1, 6) || !validList(plan.limitations, 0, 6)) throw new Error('Invalid architecture plan.');
