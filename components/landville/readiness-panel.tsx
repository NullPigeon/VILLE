...
type Status = { storage: string; sessionConfigured: boolean; privyConfigured: boolean; ai: { configured: boolean; model: string }; moduleImages: { enabled: boolean; model: string; keyPresent: boolean }; walletTransactions: { enabled: boolean; adapterConfigured: boolean; treasuryConfigured: boolean; chainId: number; mode: string }; builderEnabled: boolean; builderConfigurationPresent: boolean };
export function ReadinessPanel() {
  ...
  return <section className="lv-panel chat-sidebar-body">
    ...
    {status && <dl><dt>SUPABASE</dt><dd>{status.storage}</dd><dt>PRIVY LOGIN</dt><dd>{status.privyConfigured ? 'App ID and server secret present' : 'App ID or server secret missing'}</dd><dt>WALLET SESSION SECRET</dt><dd>{status.sessionConfigured ? 'Configured' : 'Missing or too short'}</dd><dt>SCRAPY AI</dt><dd>{status.ai.configured ? 'Key present; run live test' : 'Key missing'} · {status.ai.model}</dd><dt>MODULE IMAGES</dt><dd>{status.moduleImages.enabled && status.moduleImages.keyPresent ? 'Enabled' : 'Disabled or key missing'} · {status.moduleImages.model}</dd><dt>WALLET TRANSACTIONS</dt><dd>{status.walletTransactions.enabled ? 'Enabled' : 'Disabled'} · adapter {status.walletTransactions.adapterConfigured ? 'configured' : 'missing'} · treasury {status.walletTransactions.treasuryConfigured ? 'configured' : 'missing'} · chain {status.walletTransactions.chainId} · {status.walletTransactions.mode}</dd><dt>BUILDER</dt><dd>{status.builderEnabled ? 'Enabled' : 'Disabled'} · {status.builderConfigurationPresent ? 'Coordinator settings present, not verified' : 'Coordinator settings incomplete'}</dd></dl>}
