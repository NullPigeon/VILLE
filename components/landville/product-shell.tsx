...
export function ProductShell({ title, eyebrow, actions, children, immersive = false }: { title: string; eyebrow: string; actions?: React.ReactNode; children: React.ReactNode; immersive?: boolean }) {
  ...
  return <div className="product-root">
    <aside className={open ? 'product-rail rail-open' : 'product-rail'}>
      ...
      <nav>{nav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={pathname.startsWith(href) ? 'active' : ''} onClick={() => setOpen(false)}><Icon /> <span>{label === 'PROFILE' ? wallet.address ? 'MY PROFILE' : 'CREATE ACCOUNT' : label}</span></Link>)}{town.isAdmin && <Link href="/admin" onClick={() => setOpen(false)}><Wrench /><span>BUILD CONTROL</span></Link>}</nav>
    ...
    <div className="product-main">
      ...
      {!immersive&&<nav className="mobile-dock">{mobileNav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={pathname.startsWith(href) ? 'active' : ''}><Icon /><span>{label}</span></Link>)}{town.isAdmin && <Link href="/admin" onClick={() => setOpen(false)}><Wrench /><span>BUILD CONTROL</span></Link>}</nav>}
    </div>
  </div>;
}
