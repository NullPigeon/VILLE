...
import { Building2, CircleDollarSign, Crown, Grid2X2, Home, Menu, MessageCircle, User, Vote, Wrench, X } from 'lucide-react';
...
export function ProductShell({ title, eyebrow, actions, children, immersive = false }: { title: string; eyebrow: string; actions?: React.ReactNode; children: React.ReactNode; immersive?: boolean }) {
  ...
  const mobileNav = nav.filter((item) => !['/treasury', '/one-scrapy'].includes(item.href));
  ...
  return <div className="product-root">
    ...
    <div className="product-main">
      ...
      {!immersive&&<nav className="mobile-dock">{mobileNav.map(({ href, label, icon: Icon }) => <Link key={href} href={href === '/citizens' ? profileHref : href} className={pathname.startsWith(href) ? 'active' : ''}><Icon /><span>{label}</span></Link>)}{town.isAdmin && <Link href="/admin" onClick={() => setOpen(false)}><Wrench /><span>BUILD CONTROL</span></Link>}</nav>}
    </div>
  </div>;
}
