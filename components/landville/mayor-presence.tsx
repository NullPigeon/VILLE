...
export function MayorPresence() {
  ...
  return (
    <div className={open ? 'mayor-presence open' : 'mayor-presence'}>
      {open && <section className="mayor-presence-panel"><header><span><Bot /> MAYOR SCRAPY</span><button onClick={() => setOpen(false)} aria-label="Close Mayor presence"><X /></button></header><small>● ONLINE · TOWN CHAT</small><div>{messages.length ? messages.map((message) => <p key={message.id}><b>{message.author}</b>{message.body}</p>) : <p><b>NO BROADCASTS YET</b>The town chat is empty. Suspiciously peaceful.</p>}</div><nav><Link href="/chat">OPEN TOWN CHAT</Link><Link href="/mayor">TALK PRIVATELY</Link></nav></section>}
