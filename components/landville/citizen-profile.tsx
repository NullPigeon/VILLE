...
import { ArrowUpRight, Hammer, LogOut, User, Vote, Wallet } from 'lucide-react';
...
export function CitizenProfile({ identity }: { identity: string }) {
  ...
  const stats = {
    ...
    votes: ownReceipts.reduce((sum, receipt) => sum + receipt.weight, 0),
  };
  ...
  return <ProductShell title="CITIZEN FILE" eyebrow="WALLET IDENTITY / PUBLIC RECORD" actions={isOwnWallet?<button className="lv-button" onClick={()=>wallet.disconnectWallet()}><LogOut /> SIGN OUT</button>:undefined}>
    <section className="lv-panel profile-header"><div className="passport-avatar citizen-placeholder"><User /></div><div className="profile-identity"><small>RESIDENT ID · LV-{username.slice(0,6).toUpperCase()}</small><h2>@{username}</h2><p>{requestedWallet ? shortWallet(requestedWallet) : 'NO WALLET LINKED'}<br />{isOwnWallet ? `${wallet.snapshot?.tokenBalanceFormatted || '—'} LAND · ×${wallet.snapshot?.weight ?? '—'} CURRENT POWER` : 'HOLDINGS ARE CHECKED ONLY DURING ACTIONS'}</p></div><div className="profile-stamp">{title}</div></section>
    {!requestedWallet&&<div className="admin-warning">THIS NAME HAS NO WALLET-LINKED PUBLIC RECORD YET.</div>}
