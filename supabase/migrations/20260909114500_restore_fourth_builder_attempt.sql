-- The 45-minute lease migration accidentally restored the older three-attempt
-- claim limit. Keep the extended lease and restore the intended fourth attempt.
begin;
...
begin
  ...
  for proposal in select * from public.landville_proposals where status = 'LIVE' and closes_at <= clock_timestamp() order by closes_at, id limit 100 for update loop
    perform public.landville_transition(proposal.id, p_actor, 'LIVE', 'FINALIZE', 'The 2-hour voting window ended.');
  end loop;
