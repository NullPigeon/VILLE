-- Restore one LV-5 run after attempt 4 failed before categorized diagnostics
-- were deployed. The failed run created no GitHub branch or review artifact.
begin;
...
begin
  ...
  if job.state is distinct from 'FAILED'
    ...
    or job.error is distinct from 'Builder failed. Inspect the private workflow run before retrying.'
    or not exists (
      select 1 from public.landville_proposals where id = 'LV-5' and status = 'BUILDING'
    ) then raise exception 'LV-5 no longer matches the expected hidden attempt-4 failure';
  end if;
  ...
  insert into public.landville_build_events(proposal_id, actor_wallet, previous_status, status, note)
    values('LV-5', job.reviewed_by, 'BUILDING', 'BUILDING',
      'Attempt 4 failed before categorized diagnostics were deployed. One observed rerun was restored.');
end $$;
