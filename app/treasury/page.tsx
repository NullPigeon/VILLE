...
export default function TreasuryPage() {
  ...
  return <ProductShell title="TREASURY" eyebrow="PUBLIC FUNDS / READ-ONLY" actions={<button className="lv-button" onClick={addNetwork}><Network /> CONNECT MAINNET</button>}>
    ...
    <div className="product-grid"><section className="lv-panel"><header className="lv-panel-head"><h2><WalletCards /> ASSETS</h2><span>{treasuryAddress || 'NO ADDRESS'}</span></header><div className="asset-list"><div className="empty-state"><WalletCards /> Treasury assets will appear after a public address and chain indexer are configured.</div></div><div style={{padding:16}}><Link className="lv-button" href="/proposals">VIEW TREASURY PROPOSALS <ArrowUpRight /></Link></div></section>
      <aside className="lv-panel"><header className="lv-panel-head"><h2><CircleDollarSign /> CHAIN ADAPTER</h2><span>MAINNET / READ-ONLY</span></header><div className="chain-card"><div className="chain-status"><i /><div><b>{activeRobinhoodChain.name}</b><small>MAINNET CONFIGURED</small></div></div><div className="chain-facts"><div><span>CHAIN ID</span><b>{activeRobinhoodChain.id}</b></div><div><span>GAS TOKEN</span><b>ETH</b></div><div><span>FINALITY</span><b>STAGED L2 → L1</b></div><div><span>EXECUTION</span><b>DISABLED</b></div></div><p className="chain-note">Set the mainnet RPC and a mainnet treasury address in Vercel. Asset indexing is not connected yet.</p><a className="lv-button" href={treasuryAddress ? `${activeRobinhoodChain.explorerUrl}/address/${treasuryAddress}` : activeRobinhoodChain.explorerUrl} target="_blank" rel="noreferrer">OPEN EXPLORER <ExternalLink /></a></div></aside>
    </div>
  </ProductShell>;
}
