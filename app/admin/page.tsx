...
import { ArrowUpRight, Hammer, ShieldAlert } from 'lucide-react';
import { ProductShell } from '@/components/landville/product-shell';
import { useLandville } from '@/components/landville/provider';

export default function AdminPage() {
  const { proposals, objects, setProposalStatus } = useLandville();
  const queue = proposals.filter((proposal) => proposal.status !== 'BUILT' && proposal.status !== 'REJECTED');
  return <ProductShell title="BUILD CONTROL" eyebrow="BUILD QUEUE / HUMAN APPROVAL REQUIRED">
    <div className="admin-warning"><ShieldAlert /> RESTRICT THIS ROUTE WITH SERVER-SIDE ADMIN AUTHORIZATION BEFORE PRODUCTION.</div>
