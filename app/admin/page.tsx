...
type Selection = { proposal: ProposalRecord; action: 'PREPARE' | 'RETRY' | 'REBUILD' | 'FINALIZE' | 'REJECT' | 'RELEASE' };
...
export default function AdminPage() {
  ...
  const live = proposals.filter((proposal) => proposal.status === 'LIVE' && !ids.has(proposal.id));
  const released = proposals.filter((proposal) => proposal.status === 'BUILT' && jobs.some((job) => job.proposal_id === proposal.id));
  const queue = [...ordered, ...live, ...released];
  ...
  return <ProductShell title="BUILD CONTROL" eyebrow="REVIEW → BUILD → PR → VERIFIED RELEASE">
    {!isAdmin ? <section className="lv-panel chat-sidebar-body"><h2>ADMIN ACCESS REQUIRED</h2><p>{status === 'loading' ? 'Checking access…' : 'Sign in with an authorized operator wallet.'}</p><Link className="lv-button" href="/citizens">MY PROFILE / SIGN IN</Link></section> : <>
      ...
      <section className="lv-panel"><header className="lv-panel-head"><h2>BUILD QUEUE</h2><span>{queue.length} PROPOSALS</span></header><div style={{ overflowX: 'auto' }}><table className="build-table"><thead><tr><th>PROPOSAL</th><th>VOTES / DEADLINE</th><th>BUILDER</th><th>ACTION</th></tr></thead><tbody>{queue.map((proposal) => {
        const job = jobs.find((item) => item.proposal_id === proposal.id);
        return <tr key={proposal.id}><td><strong>{proposal.title}</strong><br />{proposal.id} · {proposal.creator}<p>{proposal.summary}</p></td><td>{proposal.yes} YES / {proposal.no} NO<br />{proposal.closesAt && new Date(proposal.closesAt).toLocaleString()}</td><td>{proposal.status}<br />{job ? `${job.state} · revision ${job.revision} · attempt ${job.attempt}/3` : 'Needs specification'}{job?.error && <p>{job.error}</p>}{job?.pr_number && <p><a href={`https://github.com/NullPigeon/VILLE/pull/${job.pr_number}`} target="_blank" rel="noreferrer">REVIEW PR #{job.pr_number}</a></p>}</td><td><div className="build-actions">
          ...
          {(job?.state === 'FAILED' || job?.state === 'REVIEW') && (proposal.status === 'BUILDING' || proposal.status === 'BUILT') && <button disabled={busy || job.attempt >= 3} onClick={() => choose(proposal, 'RETRY')}>{job.state === 'REVIEW' ? 'REBUILD UNRELEASED REVISION' : 'RETRY BUILD'}</button>}
          ...
          {proposal.status === 'BUILT' && job?.state === 'RELEASED' && <button disabled={busy || job.attempt >= 3} onClick={() => choose(proposal, 'REBUILD')}>REBUILD RELEASE</button>}
      ...
      {selected && <section className="lv-panel"><form className="proposal-form" onSubmit={submit}><h2>{selected.action} · {selected.proposal.id}</h2>
        ...
        selected.action === 'REBUILD' ? <p>Create a new immutable revision using the original voted goal and acceptance checks. The current release stays live until the replacement PR, production deployment and artifact hash are verified.</p> :
        selected.action === 'RETRY' ? <p>Inspect the previous workflow and PR first. Close or merge that PR before approving another paid attempt. A merged but unverified revision may be replaced; the current verified World object stays live. Maximum three attempts.</p> :
