...
export default function ProposalsPage() {
  ...
  return <ProductShell title="PROPOSALS" eyebrow="IMAGINE / HOLD / VOTE" actions={<Link className="lv-button primary" href="/chat"><Bot /> DISCUSS WITH SCRAPY</Link>}>
    ...
    <p className="admin-warning">Each proposal has its own {VOTING_HOURS}-hour vote. At least {MINIMUM_MODULE_VOTERS} distinct citizens must vote and YES must exceed NO. Ties and votes with fewer than {MINIMUM_MODULE_VOTERS} participants are rejected. Approved proposals are built one at a time, in voting-deadline order.</p>
    ...
    <section className="proposal-list">{visible.map((proposal) => {
      ...
      return <article className="proposal-row" id={proposal.id} key={proposal.id}>
        ...
        <div className="proposal-copy"><small>{proposal.category} · {proposal.district} · {proposal.creator}</small><h2>{proposal.title}</h2><p>{proposal.summary}</p><small>MINIMUM {MINIMUM_MODULE_VOTERS} CITIZENS · YES MUST EXCEED NO · CLOSES: {proposal.closesAt ? new Date(proposal.closesAt).toLocaleString() : '—'}</small></div>
