...
export default function Home() {
  ...
  return <main>
    ...
    <section id="top" className="hero shell">
      <div className="chaos-note chaos-note-a" aria-hidden="true">APPROVED?<b>ABSOLUTELY NOT.</b></div>
      <div className="chaos-note chaos-note-b" aria-hidden="true">LOT 13<br /><b>FOUND / NOT STOLEN</b></div>
      <div className="chaos-arrow" aria-hidden="true">↳ THIS WAY, PROBABLY</div>
    ...
    <section className="shell how chaos-section" aria-label="How Landville works"><div className="torn-caption" aria-hidden="true">PROCESS MAY VARY<br />WILDLY</div><p className="eyebrow"><i /> THE CIVIC PROCESS, ALLEGEDLY</p><div>{[['01','IMAGINE','Have a weird idea.'],['02','TALK','Tell Mayor Scrapy.'],['03','PROPOSE','Make it buildable.'],['04','VOTE','Citizens decide.'],['05','BUILD','It joins the town.'],['06','LIVE','Regret it forever.']].map(([n,title,text])=><article key={n}><span>{n}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>

    <section id="world" className="shell world chaos-section"><div className="section-stamp" aria-hidden="true">EXHIBIT A<br /><b>TERRIBLE DECISIONS</b></div><div className="heading"><div><p className="eyebrow"><i /> LIVE WORLD / LANDFILLE</p><h2>THE TOWN GOT<br />WORSE. <em>GOOD.</em></h2></div><p>Every strange thing in here was imagined by someone, voted on by everyone, then made permanent.</p></div>
    ...
    <section id="votes" className="vote-section chaos-vote"><div className="vote-scribble" aria-hidden="true">NO QUIET<br />OPINIONS</div><div className="shell vote-layout"><div className="vote-intro"><p className="eyebrow"><i /> CITIZENS DECIDE</p><h2>MAKE THEM<br /><em>VOTE.</em></h2><p>Landville does not need another committee. It needs a giant green button and people with opinions.</p><button className="button acid" onClick={() => setChat(true)}>START A PROPOSAL <ArrowUpRight /></button></div><div className="vote-stack">{votes.map(([title,author,yes,time]) => <article key={title as string}><header><span>PROPOSAL LIVE</span><time>{time}</time></header><h3>{title}</h3><p>Proposed by {author}</p><div className="meter"><i style={{width:`${yes}%`}} /></div><footer><b><em>{yes}%</em> YES</b><span>{100-(yes as number)}% NO</span><button onClick={() => setVoted(title as string)}>{voted === title ? 'VOTED ✓' : 'VOTE'} <Vote /></button></footer></article>)}</div></div></section>
    ...
    <footer className="site-footer chaos-footer"><a className="logo" href="#top">LANDVILLE<span>™</span></a><p>THIS IS NOT JUST A WEBSITE. IT’S A TOWN.</p><b>NO REFUNDS</b><span>© 2026 THE INTERNET</span></footer>
