...
import { ArrowDownRight, ArrowUpRight, Bot, Building2, ChevronRight, CircleDollarSign, Crown, MapPin, Menu, MessageSquare, Plus, Send, Sparkles, Vote, X } from 'lucide-react';
...
export default function Home() {
  ...
  return <main>
    ...
    <section id="world" className="shell world"><div className="heading"><div><p className="eyebrow"><i /> LIVE WORLD / LANDFILLE</p><h2>THE TOWN GOT<br />WORSE. <em>GOOD.</em></h2></div><p>Every strange thing in here was imagined by someone, voted on by everyone, then made permanent.</p></div>
      <div className="world-frame"><img src="/landville-reference.png" alt="The neon junkyard city of Landville" /><div className="overlay" /><b className="label casino"><MapPin /> FROG CASINO</b><b className="label museum"><MapPin /> MEME MUSEUM</b><b className="label market"><MapPin /> MARKET</b><div className="map-controls"><button aria-label="Zoom in"><Plus /></button><button aria-label="Explore map"><ArrowUpRight /></button></div><div className="world-feed"><small>LIVE BUILD FEED</small><b>3 new objects this week.</b><span>Two were useful. We’re investigating.</span></div></div>
      <div className="projects">{projects.map(([title,district,author,yes],i) => <article key={title as string}><span>0{i+1}</span><div className="building"><Building2 /></div><p>{district}</p><h3>{title}</h3><footer>BUILT BY {author}<b>{yes}% <small>VOTED YES</small></b></footer></article>)}</div>
    </section>
    ...
    {chat && <aside className="chat" aria-label="Mayor Scrapy chat"><header><div className="bot"><Bot /></div><div><b>MAYOR SCRAPY</b><small><i /> ONLINE / DOCKED</small></div><button onClick={() => setChat(false)} aria-label="Close mayor chat"><X /></button></header><div className="thread">{messages.map((m,i) => <div className={m.who === 'YOU' ? 'message you' : 'message'} key={i}><small>{m.who}</small><p>{m.text}</p></div>)}</div><div className="concept"><Sparkles /><div><small>CONCEPT READY</small><b>MAKE A TOKEN SWAP, BUT PHYSICAL</b></div><ChevronRight /></div><form onSubmit={send}><input value={input} onChange={e=>setInput(e.target.value)} placeholder="Tell Mayor what Landville needs..." /><button aria-label="Send"><Send /></button></form></aside>}
  </main>;
}
