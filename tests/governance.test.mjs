...
import { calculateVoteWeight, calculateWeeklyModuleLikes } from '../lib/governance.ts';
...
for (const decimals of [0, 6, 18]) {
  const unit = 10n ** BigInt(decimals);
  for (const [tokens, expected] of [[0n, 5], [249_999n, 5], [250_000n, 6], [1_000_000n, 9]]) {
    void test(`${tokens} SCRAPY (${decimals} decimals) gives ${expected} weekly module likes`, () => {
      assert.equal(calculateWeeklyModuleLikes(tokens * unit, decimals), expected);
    });
  }
}
