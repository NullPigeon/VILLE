...
for (const decimals of [0, 6, 18]) {
  ...
  for (const [tokens, expected] of cases) {
    test(`${tokens} SCRAPY (${decimals} decimals) gives ${expected} votes`, () => {
  ...
  for (const [threshold, weight] of [[250_000n, 2], [500_000n, 3], [1_000_000n, 5]]) {
    test(`one smallest unit below ${threshold} SCRAPY (${decimals} decimals) does not round up`, () => {
      assert.equal(calculateVoteWeight(threshold * unit - 1n, decimals), weight - 1);
    });
  }
}

test('base vote does not satisfy the existing build-request token-bonus gate', () => {
