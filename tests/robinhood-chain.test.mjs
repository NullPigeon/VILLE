...
void test('product defaults to official Robinhood mainnet, not testnet', () => {
...
void test('decimal and hexadecimal chain IDs agree', () => {
  for (const config of Object.values(robinhoodChain)) assert.equal(Number(config.hexId), config.id);
});

void test('wallet switch targets mainnet and verifies selected chain', async (t) => {
  const calls = [];
...
void test('unknown mainnet is added, explicitly switched, then checked', async (t) => {
  const calls = [];
  t.after(() => { delete globalThis.window; });
  globalThis.window = { ethereum: { request: async (call) => {
    calls.push(call);
...
void test('wallet left on testnet is rejected', async (t) => {
  t.after(() => { delete globalThis.window; });
...
void test('a rejected switch never adds another network or falls back', async (t) => {
  const calls = [];
  t.after(() => { delete globalThis.window; });
  globalThis.window = { ethereum: { request: async (call) => {
    calls.push(call);
...
void test('missing wallet reports a useful error', async (t) => {
  t.after(() => { delete globalThis.window; });
  globalThis.window = {};
